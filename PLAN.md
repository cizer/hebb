...
- **Phase 4 Stage 3 release / binary distribution** (the remaining distribution gap now the marketplace exists): the plugin marketplace delivers MCP+skill config but NOT the `hebb` binary, which must be on PATH. Public repo ⇒ `go install github.com/cizer/hebb/cmd/hebb@latest` works today for Go users; for everyone else, a tagged release with cross-compiled binaries + a Homebrew tap (`brew install cizer/hebb/hebb`). GoReleaser + a GitHub Actions release job (gated on acceptance) is the natural shape; ties off Stage 3.
