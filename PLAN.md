...
**Immediate next: a Codex adapter, or Phase 4 Stage 3 release (still deferred).** State as of this point: plugin migration **A + B1 + B2 done** and automation migration **C done**, on `main`, CI green, working tree clean. Phases 0-3 done; the plugin is adopted (see decision below).
...
**Then:** Phase 4 Stage 3 release (deferred until system testing is further along: tagged release with cross-compiled binaries + a Homebrew tap so `command: hebb` resolves on PATH); a Codex adapter (MCP config in `~/.codex/config.toml` + the workflow as `AGENTS.md` guidance; skills are Claude-only); Phase 5 cutover (OneVault, last). Possible tidy-up: the `Skills` field in `core.VaultConfig` / `DefaultVaultConfig` is now dead config (install ignores it post-B2) — drop it or leave as schema doc. Test vaults: PersonalVault (live hebb vault); OneVault untouched. `spike/hebb-plugin` is gitignored scratch; `plugin/` is canonical. Exercise the full surface against a throwaway dir with `--home`/`--data-dir`/`--launchd-dir`/`--mcp-json`.
