...
- **Persist the raw transcript, always.** When a transcript is found, the raw VTT is saved to the destination's `Artifacts/` as the canonical source, **including, especially, when a sub-agent fetched and read it during a sweep.** An agent's in-context read is not the record: if the agent returned the substance but did not save the file, re-fetch and save it before filing, so the synthesis stays auditable against source. The only acceptable miss is a genuinely empty/unavailable transcript.
- **Where the transcript lands depends on size.** Large transcripts exceed the tool read-limit and get written to a temp file: extract the transcript from there and copy it into `Artifacts/`. Small transcripts return inline (no temp file is created), so save the inline content straight to `Artifacts/`. If you do not reproduce it, record the exact re-pullable reference instead (the transcript's `transcriptContentUrl`, or its `meetingId` plus transcript `id`, with the occurrence window) so the source stays retrievable. Don't let an inline return be the reason a transcript goes unpersisted.
...
- **Search the user's name in prose, not just the mention token.** Colleagues routinely refer to the user by name ("a priority request from [name]", "I'll pick this up with [name]") rather than @-mentioning them, and a `to:me` / `<@id>` mention query misses every one of those. Always run a plain name search on the user's name (first name and surname, per the vault `CLAUDE.md`) alongside the mention/`from:`/`to:` filters. This is the single most common way a directly-relevant thread gets missed.
...
- **Conduct / interpersonal content** (a meeting or its fallout where someone is criticising or escalating about a colleague) follows vault-ingest's conduct rule: read the source directly rather than work from a sub-agent's summary, capture it balanced with the load-bearing lines near-verbatim, mark it decision-support / human-owned, and never harden a characterisation into a dossier fact. File to the person's rolling log, not the dossier body.
