...
- **Agent wiring via thin adapters:** for Claude, the hebb plugin (`plugin/`) provides the MCP server and the `vault-ingest` skill, user-level, resolving the opened vault through `HEBB_VAULT=${CLAUDE_PROJECT_DIR}`; no per-vault MCP registration or skill symlinks. For Codex, `hebb codex` merges an `[mcp_servers.hebb]` block into `~/.codex/config.toml` pinned to a vault (skills are Claude-only; vault guidance lives in the vault's `AGENTS.md`). `hebb install --mcp-json` writes a per-vault `.mcp.json` + project settings as a plugin-less fallback. The engine and MCP surface are the same across all three; only the adapter differs.
- **Shared vs per-vault:**
