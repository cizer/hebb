...
- `hebb reset`: un-wire a vault from the machine (memory link, launchd jobs, agent configs, index, and the registry entry; the global web service is retired only when the last vault is removed). Dry run by default; `--force` to apply. Never touches your notes.
