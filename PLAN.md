...
**Immediate next: B2 — consolidate skills into the plugin (finish the slim install).** State as of this point: `main` at the B1 commit, pushed, CI green, working tree clean. Phases 0-3 done; the plugin is adopted (see decision below). The plugin migration is in three slices: **A done** (graduated `plugin/` into the repo: manifest + `.mcp.json` with `HEBB_VAULT=${CLAUDE_PROJECT_DIR}` + the vault-ingest skill; spike validated live). **B1 done** (per-vault `.mcp.json` + settings are now opt-in via `hebb install --mcp-json`; default install is data-side; doctor treats a missing `.mcp.json` as ok/plugin-mode). **B2 not started** (below).

**B2 scope (skills consolidation):** default install still symlinks skills into `<vault>/.claude/skills`; remove that now the plugin delivers skills. Concretely: delete the skills block from `install.Run`; delete `install/skills.go` + `install/skills_test.go` (move the `makeSkills` test helper if still referenced by `run_test`/`doctor_test`); drop `all:skills` from `assets.go`'s embed and delete the now-dead root `skills/` dir (plugin/skills/ is the single copy); remove `checkSkills` from `install/doctor.go` and its two tests; strip skill assertions from `run_test`/`doctor_test`/`cli/install_test`/`scripts/acceptance.sh`; update the install help text and README/ARCHITECTURE. Keep `resolveAssetDir`/materialise for launchd automation + vault-template, `linkOne` (used by memory), and the `--mcp-json` path. Verify scaffold reads vault-template from embed vs data dir. After B2, install is purely data-side.

**Then:** automation scripts migration (daily-digest, action-review into `automation/`); Phase 4 Stage 3 release (deferred); a Codex adapter (MCP config + AGENTS.md); Phase 5 cutover (OneVault, last). Test vaults: PersonalVault (live hebb vault, project-scoped); OneVault untouched (its vault-ingest relocated to its own `.claude/skills`). `spike/hebb-plugin` is gitignored scratch; `plugin/` is canonical. Exercise the full surface against a throwaway dir with `--home`/`--data-dir`/`--launchd-dir`/`--mcp-json`.
