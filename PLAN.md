...
- move the real automation `vault/bin/*` into `automation/` (today placeholders); they then embed into the binary and install materialises + launchd renders them (skills already done: they live in `plugin/skills/`, delivered by the plugin)
...
**Immediate next: C — migrate the automation scripts into `automation/`.** State as of this point: plugin migration **A + B1 + B2 all done**, on `main`, CI green, working tree clean. Phases 0-3 done; the plugin is adopted (see decision below).

Plugin migration recap: **A** graduated `plugin/` into the repo (manifest + `.mcp.json` with `HEBB_VAULT=${CLAUDE_PROJECT_DIR}` + the vault-ingest skill; spike validated live). **B1** made per-vault `.mcp.json` + settings opt-in via `hebb install --mcp-json` (default install is data-side; doctor treats a missing `.mcp.json` as ok/plugin-mode). **B2** removed skills from install entirely: deleted the skills block from `install.Run`, `install/skills.go`/`_test.go`, `checkSkills` + its two doctor tests, and the dead root `skills/` dir; dropped `all:skills` from `assets.go`'s embed (now `automation` + `vault-template` only); rewrote `run_test`/`doctor_test`/`cli/install_test` and `scripts/acceptance.sh` to assert install never creates `<vault>/.claude/skills`; updated install/new/doctor help text and README/ARCHITECTURE. `plugin/skills/vault-ingest` is now the single skill copy. **Install is purely data-side: `.hebb/config.toml`, memory symlink, first index, plus launchd jobs and automation materialise.** `resolveAssetDir`/materialise (now automation-only) and `linkOne` (memory) are retained. Acceptance is 28/28; scaffold reads `vault-template` straight from the embedded FS (not the data dir).

**C scope (automation migration):** the real automation scripts (daily-digest, action-review) still live at `vault/bin/*` on the live machine; today `automation/` holds placeholders. Pull the generic forms into `hebb/automation/`, parameterised by vault (no hardcoded OneVault paths), so they embed into the binary and `install` materialises them to the data dir for launchd. `VaultJobs` already gates digest/action-review on `<assetDir>/automation/<script>` existing, so adding the scripts lights up those jobs. Keep OneVault untouched.

**Then:** Phase 4 Stage 3 release (deferred until system testing is further along); a Codex adapter (MCP config in `~/.codex/config.toml` + the workflow as `AGENTS.md`); Phase 5 cutover (OneVault, last). Test vaults: PersonalVault (live hebb vault); OneVault untouched (its vault-ingest relocated to its own `.claude/skills`). `spike/hebb-plugin` is gitignored scratch; `plugin/` is canonical. Exercise the full surface against a throwaway dir with `--home`/`--data-dir`/`--launchd-dir`/`--mcp-json`.
