...
- ✅ **Stage 3 — release (the deploy): proven.** `.goreleaser.yaml` + `.github/workflows/release.yml` + `RELEASING.md`. Validated locally (`goreleaser check` + `--snapshot`, all 4 targets) AND end-to-end on a real tag: **`v0.1.0` was cut while the repo is private** — the workflow went green (test 33s, release 56s) and published a GitHub release with the 4 tar.gz archives + checksums.txt; the darwin/arm64 asset reports `hebb version 0.1.0` (tag→ldflags stamping confirmed). Homebrew tap (`cizer/homebrew-hebb`) configured-but-commented. **Pending: flip repo public** (`gh repo edit cizer/hebb --visibility public` - Richie's call; gates `go install` + public marketplace) and a **LICENSE** (Legal). Tidy-up: bump `goreleaser-action@v6` (runs on Node 20, force-migrated to Node 24 on 16-06-2026) - non-blocking. Optional later: npm, macOS signing/notarization.
...
**Next up (queued):**
- ✅ **`hebb reset` (vault teardown) — DONE.** `install.Teardown(opts)` + `cli/reset.go`: dry-run by default (`--force` to apply) un-wires memory symlink (only if it resolves into this vault; uses the PATH slug, not the name slug), launchd plists `local.hebb.<name-slug>.*` (+ best-effort `Bootout`), the codex `[mcp_servers.<name>]` block (`RemoveCodexConfig`/`removeCodexBlock` added to `install/codex.go`), the opt-in `.mcp.json` (only if hebb-generated), and clears `.hebb/index.db` (`--keep-index` to skip). NEVER touches notes, `.hebb/memory`, `.hebb/config.toml` (the `--purge` idea was dropped - no content-destroying mode). Hermetic tests cover dry-run/force/keep-index/foreign-link/foreign-mcp.json/bare-vault; live dry-run verified against UATVault.
...
- **Go public to finish distribution.** Stage 3 is proven (private `v0.1.0` release is green). The only remaining step to make hebb installable by anyone: `gh repo edit cizer/hebb --visibility public` (Richie's call + LICENSE/Legal). Once public, `go install github.com/cizer/hebb/cmd/hebb@latest`, the `v0.1.0` release downloads, and `/plugin marketplace add cizer/hebb` all work for everyone. Then optionally enable the Homebrew tap (create `cizer/homebrew-hebb` + `HOMEBREW_TAP_GITHUB_TOKEN`, uncomment `brews:`) and bump `goreleaser-action`.
